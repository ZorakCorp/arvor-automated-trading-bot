...
@dataclass(frozen=True)
class NestalScore:
    """Objective Nestal metrics from Hyperliquid candles (5m + 15m trends)."""
    ...
    def trends_aligned(self, action: str) -> bool:
        """Nestal rule: 5m (micro) and 15m (meso) must agree."""
        required = self.required_trend(action)
        if required is None:
            return True
        for label in (self.micro_trend, self.meso_trend):
    ...
    def alignment_label(self) -> str:
        if (
            self.micro_trend == self.meso_trend
    ...
    def confidence_for(self, action: str) -> float:
        """Nestal formula: 40% fidelity + 40% if both trends align + 20% base."""
...
def fetch_nestal_score(client: Any) -> NestalScore | None:
    """Load 5m and 15m candles and compute Nestal metrics."""
    try:
    ...
    score = compute_nestal_score(bars_5m, meso_trend=meso)
    if score is None:
    ...
    logger.info(
        "Nestal score (computed): micro=%s meso(15m)=%s alignment=%s "
